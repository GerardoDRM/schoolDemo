angular.module('SchoolApp').controller('CourseController', ['$scope', '$http', '$compile', '$mdDialog', '$mdMedia', function($scope, $http, $compile, $mdDialog, $mdMedia) {
  ...
  $scope.generatePassword = function() {
    ...
    $scope.cou.password = randomstring;
  }
  ...
  $scope.createCourse = function() {
    ...
    var l = $scope.cou.level;
    for (var level in l) {
      if (l[level]) {
        $scope.cou.levelList.push(level);
      }
    }
    // Adding object to global array
    // This tip will save a server request
    $scope.cours.push($scope.cou);
    // Update UI
  ...
  function _addCourse(cou) {
    var levels = "";
    var courses = "";
    for (var i in cou.levelList) {
      levels += $scope.levelRef[cou.levelList[i]] + ", ";
    }
